...
% Tüm bölümler yazılmıştır. Metindeki her sayı results/tables/ altındaki
% üretilmiş özetlerden alınmıştır; sayı değiştirilecekse önce
% ./scripts/run-all-experiments.sh koşulmalıdır.
%
...
\begin{abstract}
Docker Compose dosyaları yalnızca insanlar tarafından değil, dağıtım
otomasyonu tarafından da düzenlenmektedir. Böyle bir düzenleme yalnızca
YAML ayrıştırmaktan ibaret değildir: dosyada aracın modellemediği alanlar,
kullanıcı tanımlı \texttt{x-*} uzantıları, kısa ve uzun sözdizimi
varyantları ve anlam taşıyan tırnak tercihleri bulunur; naif bir
ayrıştır--değiştir--serileştir akışı hedeflenen alanın ötesinde değişiklik
ya da bilgi kaybı üretir. Python ve JavaScript ekosistemlerinde biçim
koruyan YAML kütüphaneleri bulunmasına karşın, C++ tarafında Compose'a
özgü bir düzenleme API'si ve koruma garantisi belgelenmiş bir YAML
kütüphanesi bulamadık. Bu bildiride, Compose kavramlarını yansıtan tipli
bir C++ API sunan ve düzenlemeleri düğüm ağacında yerinde yapan
ComposeMorph'u sunuyoruz. 647 gerçek dünya ve 38 kontrollü Compose dosyası
üzerinde yedi deneyle doğruluk, koruma, değişiklik yerelliği, ileriye
dönük uyumluluk ve performansı ölçtük. Bilinmeyen özelliklerin ve
\texttt{x-*} uzantılarının tamamı korunmaktadır. Değerlendirme ayrıca
...
bedeli getirmemektedir. Tüm veri kümeleri, betikler ve ham sonuçlar tek
...
Bölüm~\ref{sec:yaklasim}'de anlatılan tırnak koruyan serileştirme ile
gidermemizi sağlamıştır.
...
\texttt{x-*} uzantı alanları dokunulmadan kalır. Kaydetme, kaynakta tırnaklı
yazılmış skalerleri tırnaklı olarak yeniden üreten ve böylece değerlerin
YAML tiplerini koruyan bir serileştirici üzerinden yapılır. Modellenmemiş
alanlar için nokta-yollu (dot-path) genel bir özellik API'si, dosya
bütünlüğü için de geçici dosya üzerinden atomik kaydetme sağlanır.
...
Gerçek dosyalara kontrollü olarak eklenen bilinmeyen özellikler ve
\texttt{x-*} uzantı alanları, ilgisiz bir alan değiştirildikten sonra tüm
vakalarda korunmuştur (802/802 bilinmeyen özellik ve 1.203/1.203 uzantı
alanı örneği). \texttt{docker compose config} tarafından geçerli kabul
...
yapan temel çizgide bu oran \%38,1'de (340/892) kalmaktadır. Korpustaki
9.807 tırnaklı skaler değerin hiçbiri tip değiştirmemiş, temel çizgide ise
452 dosyada 2.570 değer tip değiştirmiştir; çıktının girdiyle aynı veriye
çözümlenme oranı \%33'ten \%100'e çıkmaktadır. Tırnak koruyan serileştirici
...
karşılaştırma ikinci bir katkıyı ortaya koyar: aynı düzenlemeleri ilgili alt
ağacı yeniden inşa ederek yapan, makul görünen ham yaml-cpp kodu ölçülebilen
hiçbir vakada kardeş alanları koruyamamıştır (0/149, 0/39, 0/116), aynı
düzenlemeler ComposeMorph'un tip-farkında API'si üzerinden yapıldığında ise
tüm vakalarda korunmuştur.
% Sayı kaynakları: results/tables/preservation_summary.md (802/802,
% 1203/1203); validation_summary.md (892/892 vs 340/892);
% quote_normalization_analysis.md (9807 tırnaklı skaler, 0 vs 2570/452);
% comparison_summary.md (PyYAML özdeşliği 100/100 vs 33/100; naive
% 0/149, 0/39, 0/116); roundtrip_dataset_b_summary.md (26/647 = %4,02);
% performance_summary.md (serileştirici oranları 0,966/0,944/0,944/0,785).
% NOT: mutlak süreler makine yüküne duyarlı; gönderim öncesi Deney 7
% sakin bir makinede yeniden koşulmalı (bkz. Bölüm~\ref{sec:yontem}).
...
\begin{enumerate}
  ...
  \item C++ YAML serileştirmesinde sessiz tip değişimine yol açan bir hata
    sınıfının teşhisi ve giderilmesi: yaml-cpp'nin serileştiricisi, tırnaklı
    bir skalerin taşıdığı \texttt{!} etiketini yok sayarak değeri tırnaksız
    yazmakta, böylece string değerler sayıya veya boolean'a dönüşmekte ve
    dosya \texttt{docker compose config} tarafından reddedilmektedir.
    Etiketi dikkate alan serileştirme bu davranışı ortadan kaldırmakta,
    etkisi önce/sonra ölçülmektedir (geçerlilik \%38,1 $\rightarrow$ \%100).
    Aynı değerlendirme, kısa sözdizimli \texttt{environment}, \texttt{labels}
    ve \texttt{extra\_hosts} listelerinin düzenleme sırasında eşleme
    (mapping) yapısına dönüştürülmesi biçimindeki ikinci bir hatayı da
    ortaya çıkarmış ve gidermiştir.
...
bölümler ile \texttt{x-} önekli \emph{uzantı alanları} bulunur. Uzantı
...
Bir yapılandırma dosyasını programatik olarak değiştirmenin olağan yolu
\emph{ayrıştır $\rightarrow$ değiştir $\rightarrow$ serileştir} akışıdır.
Bu akışın sadakati, kullanılan ara temsile bağlıdır. Somut sözdizimi ağacı
(CST) tutan araçlar kaynağın her karakterini saklar ve yalnızca dokunulan
aralığı yeniden yazar; ruamel.yaml~\cite{ruamelyaml} ve
eemeli/yaml~\cite{eemeliyaml} bu yaklaşımla yorumları ve biçimi korur.
Buna karşılık yaml-cpp~\cite{yamlcpp} gibi kütüphaneler belgeyi değiştirilebilir
bir \emph{düğüm ağacına} ayrıştırır: yapı, anahtar sırası ve değerler
korunur, ancak sunum bilgisinin bir kısmı ayrıştırmada kaybolur ve dosya
kaydedilirken tüm belge yeniden üretilir. Bu, hedeflenen alanın dışındaki
satırların da değişmesine (configuration churn) ve --- Bölüm~\ref{sec:giris}'te
gösterildiği gibi --- değerlerin tipinin sessizce değişmesine yol açabilir.
...
Bu kayıt, bu bildiride ölçtüğümüz hata sınıfının C++ serileştiricilerinde
daha önce de görüldüğünü göstermesi bakımından anlamlıdır.
...
örüntülerini kataloglamıştır~\cite{eng2023patterns}. Bu çalışmalar,
...
Bildiğimiz kadarıyla, Docker Compose'a özgü tip-farkında düzenlemeyi ve
ölçülmüş ileriye dönük uyumlu bilinmeyen-özellik korumasını bir arada sunan
bir C++ kütüphanesi bulunmamaktadır. Bu ifadeyi, taramamızın desteklediği
sınırlar içinde kullanıyoruz: ComposeMorph yorum veya tam biçim koruma
konusunda diğer ekosistemlerdeki olgun araçlardan daha iyi değildir ve
bunu amaçlamaz (Bölüm~\ref{sec:kisitlamalar}).
...
kaydeder. \texttt{ComposeFile::service(ad)} çağrısı bir \texttt{Service}
nesnesi döndürür; \texttt{Service} de Compose kavramlarını doğrudan
yansıtan alt nesneler sunar: \texttt{Environment} (ortam değişkenleri ve
\texttt{labels}), \texttt{Ports}, \texttt{Volumes}, \texttt{Networks},
\texttt{ExtraHosts}, \texttt{BuildConfig}, \texttt{HealthCheck},
\texttt{DependsOn} ve \texttt{DeployConfig}. Üst seviyede ayrıca
\texttt{NetworkCollection}, \texttt{VolumeCollection},
\texttt{ConfigCollection} ve \texttt{SecretCollection} bulunur.
...
\subsection{Tipli API ile genel API'nin ilişkisi}
...
compose.service("api").setImage("company/api:2.0");
\end{quote}

Modellenmemiş her alan için aynı nesne, nokta-yollu genel bir özellik
API'si sağlar:

\begin{quote}\ttfamily\small
api.set("x-company-security.hsm", "enabled");\\
auto v = api.get<std::string>("logging.options.max-file");\\
api.remove("deploy.placement.constraints");
\end{quote}

İki API aynı ağaç üzerinde çalışır ve birbirinin yerine geçebilir: tipli
işlemler, genel API'nin yapacağı düğüm yazma işleminin adlandırılmış ve
...
güncellenmesi gerekmez; alan genel API ile okunup yazılabilir, hiç
dokunulmasa bile dosyada aynen kalır.

\subsection{Kısa ve uzun sözdizimi}
\texttt{Environment}, \texttt{ExtraHosts} ve \texttt{Volumes} gibi sınıflar,
altlarındaki düğümün liste mi yoksa eşleme mi olduğunu çalışma zamanında
tespit eder ve dosyanın mevcut biçimini korur. Örneğin
\texttt{environment} kısa sözdiziminde (\texttt{- KEY=value} girdileri)
yazılmışsa, \texttt{environment().set()} çağrısı ilgili girdiyi yerinde
günceller ya da listenin sonuna yeni bir girdi ekler; bölümü eşlemeye
dönüştürmez. \texttt{Ports} ve \texttt{Networks} yardımcılarında bu
dönüşüm henüz eksiktir (Bölüm~\ref{sec:kisitlamalar}).

\subsection{Hata yönetimi ve güvenli kaydetme}
Bulunamayan servis, ayrıştırılamayan dosya ve şema ihlali gibi durumlar
ayrı istisna türleriyle (\texttt{ParseException},
\texttt{ServiceNotFoundException}, \texttt{ValidationException}) bildirilir.
...
olan --- skalerin kaynakta tırnaklı yazılıp yazılmadığını gösteren
\emph{belirtisiz etiket}: tırnaklı skalerler \texttt{!}, tırnaksız
(plain) skalerler \texttt{?} etiketi taşır. Ayrıştırmada kaybolan bilgi
ise yorumlar, boş satırlar, girinti genişliği, tek/çift tırnak ayrımı ve
anchor adlarıdır.

\subsection{Değişiklik izleme: yok}
...
sürümünde \texttt{IsValidPlainScalar} işlevi değeri null benzeri dizgilere
(\texttt{null}, \texttt{\textasciitilde}, boş dizgi) ve sözdizimsel olarak
sakıncalı karakterlere karşı sınar, fakat boolean veya sayı gibi
görünmesini dikkate almaz. Ayrıca ayrıştırmada üretilen \texttt{!} etiketi
serileştirmede yok sayılır. Sonuç, Bölüm~\ref{sec:giris}'teki davranıştır:
...
\begin{enumerate}
  ...
  \item \textbf{Skaler yazma kuralı.} Düğüm \texttt{!} etiketi taşıyorsa
    (yani kaynakta tırnaklıydıysa) değer çift tırnakla yazılır; aksi
...
Tek tırnaklı skalerler çift tırnakla yazılır. Bu davranışların hepsi
Bölüm~\ref{sec:bulgular} ve Bölüm~\ref{sec:kisitlamalar}'da ölçülmüş
...
(toplam 557 satır) ve sekiz Python betiği (toplam yaklaşık 3.970 satır)
içerir. Depo, görev tanımındaki yapıyı izler: \texttt{src/} ve
...
\begin{description}
  \item[RQ1 (Doğruluk)] Kütüphane gerçek Compose dosyalarını ayrıştırıp
    değiştirdikten sonra geçerli Compose dosyaları üretiyor mu?
  \item[RQ2 (Koruma)] Yalnızca belirli bir alan değiştirildiğinde,
    değişiklikle ilgisi olmayan yapılandırma bilgisinin ne kadarı korunuyor?
  \item[RQ3 (Değişiklik yerelliği)] Hedeflenen alan değiştirilirken dosyada
    ne kadar gereksiz değişiklik oluşuyor?
  \item[RQ4 (İleriye dönük uyumluluk)] Nesne modelinde tanımlanmamış
    Compose özellikleri kaybedilmeden taşınıyor mu?
  \item[RQ5 (Performans)] Yükleme, değiştirme ve serileştirme maliyeti
    gerçek kullanım için kabul edilebilir mi?
\end{description}
...
\texttt{devices}, \texttt{x-*} uzantıları, bilinmeyen özellikler ve iç içe
...
otomatik olarak toplanmış 647 Compose dosyasından oluşur. Toplama ve
analiz betikleri depoda yayımlanmıştır. Korpus, literatürdeki gözlemlerle
uyumlu biçimde küçük dosyalara eğilimlidir: satır sayısı medyanı 39
(en büyük dosya 2.591 satır), servis sayısı medyanı 2 (en fazla 56), üst
seviye bölüm sayısı medyanı 2, YAML iç içelik derinliği medyanı 4 ve
uzantı alanı sayısı medyanı 0'dır. Son değer, uzantı ve bilinmeyen alan
deneylerinde neden kontrollü işaretçi enjeksiyonuna başvurduğumuzu
açıklar: bu alanlar gerçek dosyalarda doğal olarak nadirdir.
...
özellik üzerinde), (3--4) bilinmeyen özellik ve \texttt{x-*} uzantı
...
ilgili alt ağacı yeniden inşa ederek yapan \emph{naif} ham yaml-cpp kodu.
...
oracle ile ölçülür: \texttt{docker compose config} (Compose şemasının
...
özetlerden alınmıştır. ``Temel çizgi'' sütunları, aynı düzenlemeleri
...
\begin{tabular}{lcc}
\toprule
Çıktı & ComposeMorph & Temel çizgi \\
\midrule
...
Temel çizgide geçersizleşen 552 çıktının 548'i, tırnaklı bir skalerin
...
vakaların tamamı ortadan kalkmıştır. Veri Kümesi A'da da sonuç aynıdır
(64/64'e karşı 62/64).
...
sonra tüm vakalarda korunmuştur: 802/802 bilinmeyen özellik ve
1.203/1.203 \texttt{x-*} uzantı alanı örneği (Veri Kümesi A'da 96/96 ve
...
bunların hiçbiri tip değiştirmemiştir. Temel çizgide ise 452 dosyada
...
öncesi \%0,77 ve 0,250). Kalan farklar yorumların ve boş satırların
kaybından, girinti normalizasyonundan ve tek tırnağın çift tırnağa
dönüşmesinden kaynaklanır. Anchor/alias ve birleştirme anahtarı yapıları
...
On ayrı özellik üzerinde 1.467 düzenleme senaryosu koşulmuştur. Ham
yerellik oranının medyanı özelliğe göre 0,03--0,13 arasındadır; bu düşük
değerler düzenlemenin kendisinden değil, kaydetmede yorum ve boş satır
kaybından doğan taban gürültüden kaynaklanır. Aynı dosyanın değişiklik
yapılmadan kaydedilmesindeki gürültü düşüldüğünde, on özelliğin dokuzunda
düzeltilmiş yerellik oranının medyanı 1,0'dır: düzenleme gerçekten de tek
satırla sınırlı kalmaktadır. Tek istisna \texttt{extra\_hosts}'a girdi
eklemedir (medyan 0,5). Şekil~\ref{fig:locality} dağılımı göstermektedir.
...
(1 vaka). Bu sınırlar Bölüm~\ref{sec:kisitlamalar}'da açıkça
raporlanmaktadır.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{modification_change_locality}
\caption{Özelliğe göre düzeltilmiş değişiklik yerelliği oranı
(Veri Kümesi B). 1,0 değeri, düzenlemenin yalnızca hedeflenen satırı
...
uzantılar hem de gelecekteki spesifikasyon alanlarını temsil eden
\texttt{x-} öneksiz bilinmeyen özellikler için tam olarak korunmuştur
...
resmî mekanizma \texttt{x-*} uzantılarıdır ve kütüphane bu mekanizmayı
eksiksiz desteklemektedir.

\subsection{RQ5 --- Performans}
Tablo~\ref{tab:perf}, dosya boyutuna göre yükle--değiştir--kaydet
sürelerinin medyanlarını vermektedir. Değiştirme adımının maliyeti diğer
...
\begin{table}[t]
\caption{Yükle--değiştir--kaydet süreleri, medyan (Veri Kümesi B, dosya
başına 30 yineleme)}
\label{tab:perf}
\centering
\begin{tabular}{lrrrr}
...
\end{tabular}
\smallskip
\footnotesize Süreler milisaniyedir.
\end{table}
...
(Wilcoxon işaretli sıralar testi, $p < 10^{-10}$). Tek ek maliyet, API
üzerinden yazılan değerlerin belirsizlik sınamasında kullanılan düzenli
ifadenin süreç başına bir kez derlenmesidir (yaklaşık 1,7~ms); bu maliyet
ölçümlerde ilk yinelemeye düşer ve ortalama ile p95 değerlerini yukarı
çeker.
...
alt ağacı yeniden inşa eden naif kullanım, ölçülebilen hiçbir vakada
...
(149/149, 39/39, 116/116). Aynı desen işaretçi alanlarında da görülür:
naif kullanım servis düzeyindeki 200 işaretçinin tamamını kaybederken üst
seviyedekilerin tamamını korur; hasar, üzerine yazılan alt ağaçla sınırlıdır.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{comparison_yamlcpp}
...
ağacı yeniden inşa eden naif kullanım hiçbir vakada kardeş alanları
koruyamamaktadır.}
...
22, \texttt{env} için 18 satır). Buna karşılık naif ve hatalı sürüm
\texttt{image} için 5, \texttt{hostname} için 3 satırdır: yanlış kod,
doğru koddan daha fazla emek gerektirmemektedir.
...
Bulgunun ikinci yüzü, hangi oracle ile bakıldığının sonucu değiştirmesidir.
...
girintisini --- olduğu gibi korumak gerekiyorsa, bu kütüphane uygun araç
...
yaml-cpp kodu, ComposeMorph ile aynı koruma davranışını üretebilir; yani
koruma, sihirli bir algoritmanın değil, doğru düzenleme tekniğinin
sonucudur. Ancak bu tekniği her çağrı yerinde doğru uygulamak
programcının sorumluluğundadır ve yanlış uygulamak daha fazla emek
gerektirmez: alt ağacı yeniden inşa eden sürüm hem daha kısadır hem de
derlenir; kaybı yalnızca çalışma zamanında, üstelik sessizce görülür.
Tipli API'nin katkısı, doğru olanı varsayılan yol hâline getirmesidir.
Aynı gözlem kısa sözdizimi hatasında da doğrulanmıştır: liste biçimli
\texttt{environment} bölümünü eşlemeye çeviren davranış, ham kullanımda
da kütüphanenin ilk sürümünde de aynı biçimde ortaya çıkmıştır.
...
alanların nadir olması (uzantı alanı sayısı medyanı 0) nedeniyle
enjeksiyona başvurulmuştur, dolayısıyla ölçülen koruma oranı bu kalıplar
için geçerlidir. Karşılaştırmadaki ``naif'' ham yaml-cpp kodu da bizim
...
uyumludur~\cite{ibrahim2021dockercompose,eng2023patterns}; yine de 2.000
satırın üzerinde yalnızca iki dosya bulunduğundan, en büyük boyut grubuna
ait performans sayıları küçük bir örnekleme dayanmaktadır. Veri Kümesi A
...
oracle ile birlikte raporladık: \texttt{docker compose config} ve PyYAML
...
katılığı değişirse, Bölüm~\ref{sec:bulgular}'daki temel çizgi oranları da
değişir. Aynı biçimde, yaml-cpp ileride tırnak bilgisini serileştirmede
kullanmaya başlarsa temel çizgi ile ComposeMorph arasındaki fark
...
\begin{itemize}
  ...
  \item \textbf{\texttt{x-} öneksiz bilinmeyen alanlar şemaya aykırıdır.}
    ...
    \texttt{x-} önekli uzantılara izin verdiğinden dosya
    \texttt{docker compose config} tarafından reddedilir. Bu, kütüphaneden
    bağımsız bir spesifikasyon kısıtıdır.
  \item \textbf{Doğrulama yüzeyseldir.} Kütüphane içindeki
    \texttt{validate()} yalnızca birkaç iş kuralını denetler; şema
    doğrulaması dış araca (\texttt{docker compose config}) bırakılmıştır.
...
alanlar, kullanıcı uzantıları, farklı sözdizimi gösterimleri ve anlam
...
bilinmeyen özelliklerin ve \texttt{x-*} uzantılarının tamamı korunmuştur.

...
% pdfLaTeX (Overleaf varsayılanı) 8 bit T1 fontlarla, XeLaTeX/LuaLaTeX ise
% OpenType fontlarla derlenir; aksi hâlde ı, ş, ğ gibi harfler kutucuk olur.
\usepackage{iftex}
\ifPDFTeX
...
24 dosya Compose özelliklerini tek tek (build, kısa ve uzun sözdizimli
environment/ports/volumes, networks, depends\_on, healthcheck, deploy,
configs, secrets, labels, profiles, logging, sysctls, ulimits, devices,
\texttt{x-*} uzantıları, bilinmeyen özellikler ve iç içe genel özellikler)
izole eder; 2 dosya bunları birlikte içerir; 10 dosya sınır durumlarını
(anchor/alias ve birleştirme anahtarı, tırnaklı skaler biçimleri, yalnızca
build içeren servis, Unicode içerik, null ve boş değerler, external
kaynaklar, extends, include, yalnızca büyük/küçük harfle ayrışan
anahtarlar) sınar; 2 dosya ise koruma deneylerinin işaretçi kalıplarını
taşır.
...
koruyamamaktadır (\%0). Dikkatli kullanımdaki eksik,

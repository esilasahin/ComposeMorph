...
özelliğin ve 1.203 \texttt{x-*} uzantı alanının tamamı korunmaktadır.
...
0/39, 0/116); aynı düzenlemeler ComposeMorph'un tip-farkında API'si
üzerinden yapıldığında tüm vakalarda korunmuştur.
...
\begin{enumerate}
  ...
  \item C++ YAML serileştirmesinde sessiz tip değişimine yol açan bir hata
    ...
    ortaya çıkarmış ve gidermiştir (Bölüm~\ref{sec:bulgular}).
...
satırların da değişmesine (configuration churn) ve --- giriş bölümünde
...
\texttt{Environment} (\texttt{labels} dahil) ve \texttt{ExtraHosts}
sınıfları, altlarındaki düğümün liste mi yoksa eşleme mi olduğunu çalışma
zamanında tespit eder ve dosyanın mevcut biçimini korur. Örneğin
...
dönüştürmez. \texttt{Ports}, \texttt{Volumes} ve \texttt{Networks}
...
(toplam 557 satır) ve on bir Python betiği (toplam yaklaşık 3.970 satır)
...
derinliği medyanı 4 ve uzantı alanı sayısı medyanı 0'dır. Son değer, uzantı
ve bilinmeyen alan deneylerinde neden kontrollü işaretçi enjeksiyonuna
...
çıktılarının tamamı geçerlidir (64/64); temel çizgide yalnızca tırnaklı
...
\toprule
Özellik & N & Başarılı & Değişen satır & Ham oran & Düzelt.\ oran \\
\midrule
...
\texttt{results/tables/paper\_supplementary\_stats.md}; düzeltme öncesi ham
çıktı: \texttt{results/pre-fix/raw/modification\_dataset\_b.csv}.}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{comparison_yamlcpp_tr}
...
\texttt{env} düzenlemesinde bölümün tamamını yeniden kuran naif sürüm
(6 satır), doğru sürümün üçte biri uzunluğundadır. \texttt{image} ve
\texttt{hostname} için ise doğru sürüm tek satırdır ve naif sürüm (5 ve 3
satır) daha uzundur.
...
aradaki tek fark serileştiricinin tırnak kararıdır. Yani koruma, sihirli
bir algoritmanın değil, doğru düzenleme tekniğinin sonucudur. Ancak bu
tekniği her çağrı yerinde doğru uygulamak programcının sorumluluğundadır
...
\texttt{environment} bölümünü eşlemeye çeviren davranış, naif ham
...
uyumludur~\cite{ibrahim2021dockercompose,eng2023patterns}. Ancak büyük
...
katılığı değişirse, bulgulardaki (Bölüm~\ref{sec:bulgular}) temel çizgi
...
dosyalara eklenen bilinmeyen özelliklerin ve \texttt{x-*} uzantılarının
tamamı korunmuştur.

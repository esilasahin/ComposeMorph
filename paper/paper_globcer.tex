% Authoritative source for the GLOBCER'26 submission; edit this file directly.
...
% babel, \begin{document} anında başlığı Türkçe varsayılana döndürdüğü için
% şablondaki büyük harfli başlık yeniden atanır.
\addto\captionsturkish{\renewcommand{\refname}{KAYNAKLAR}}

% Resmî GLOBCER'26 Word şablonundaki üstbilgi renkleri ve logo.
\definecolor{globcerblue}{HTML}{0070C0}
\definecolor{globcerdate}{HTML}{820000}
\definecolor{globcerconf}{HTML}{990000}
\definecolor{globcerconfblue}{HTML}{4472C4}

\fancypagestyle{globcerfirst}{%
  \fancyhf{}%
  \fancyhead[L]{\smash{\raisebox{-0.45cm}{%
    \includegraphics[height=2.1cm]{globcer26_logo.png}}}}%
  \fancyhead[R]{\textbf{\textcolor{globcerblue}{GLOBCER'26},
    \textcolor{globcerdate}{14--17 October 2026}}}%
  \renewcommand{\headrulewidth}{0pt}%
}
\pagestyle{fancy}
\setlength{\headheight}{15pt}
\fancyhf{}
\fancyhead[C]{\itshape\textcolor{globcerconf}{6\textsuperscript{th} Global
Conference on Engineering Research} \textcolor{globcerconfblue}{(GLOBCER'26)}}
\renewcommand{\headrulewidth}{0pt}
...
\noindent\textbf{Özet} Docker Compose dosyaları, geliştiricilerin yanı sıra CI/CD hatları ve dağıtım araçları tarafından da programatik olarak değiştirilmektedir. Bu dosyaları genel amaçlı YAML belgeleri gibi ele alan ayrıştır--değiştir--serileştir döngüleri; kullanıcı tanımlı \texttt{x-*} alanlarını, yeni Compose sürümleriyle gelen modellenmemiş özellikleri, kısa ve uzun sözdizimi biçimlerini ve YAML tipini etkileyen tırnak bilgisini kaybedebilir. İncelediğimiz C++ kütüphaneleri arasında, Compose alanları için sınıf tabanlı bir API sunarken bilinmeyen ve \texttt{x-*} alanlarının korunmasını sistematik round-trip deneyleriyle doğrulayan bir kütüphaneye rastlanmamıştır. Bu bildiride, Compose alanlarına tip-farkında erişim sağlayan, değişiklikleri mevcut YAML düğüm ağacı üzerinde gerçekleştiren ve dosyaları atomik olarak kaydeden C++20 kütüphanesi ComposeMorph tanıtılmaktadır. Yaklaşım; 38 kontrollü ve GitHub'dan derlenen 647 gerçek dünya Compose dosyası üzerinde yürütülen yedi deneyle doğruluk, yapı koruma, değişiklik yerelliği, Docker Compose CLI doğrulaması ve performans açısından değerlendirilmiştir. Hedeflenen tek alanlık bir düzenlemede dosyalara eklenen 802 bilinmeyen özelliğin ve 1.203 \texttt{x-*} alanının tamamı korunmuş; kaydetmenin taban gürültüsü düşüldüğünde ölçülen on özelliğin tamamında düzenlemenin kendi katkısı tek satırla sınırlı kalmıştır. Değerlendirme ayrıca yaml-cpp tabanlı serileştirmede sessiz bir tip değişimi kusuru ortaya çıkarmıştır: tırnaklı skalerler tırnaksız yazıldığında docker compose config çıktıların yalnızca \%38,1'ini kabul ederken, ayrıştırıcının sakladığı tırnak bilgisini kullanan serileştirme başarı oranını \%100'e (892/892) çıkarmıştır. Bu düzeltme ölçülebilir bir performans bedeli oluşturmamış; eşleşik ölçümde tırnak koruyan serileştirici yaml-cpp'nin kendi serileştiricisinden \%4--18 daha hızlı çalışmış ve en büyük dosyalarda yükle--değiştir--kaydet toplamının medyanı yaklaşık 15,23 milisaniye olmuştur. Koruma metinsel değil yapısaldır: yorumlar ve boş satırlar korunmadığından, değişiklik yapılmadan kaydedilen dosyaların yalnızca \%4,0'ü bayt düzeyinde aynı kalmıştır. Düzenlenen alt ağacı yeniden kuran ham yaml-cpp kodunun ölçülen hiçbir vakada servisin kardeş alanlarını koruyamaması, bu kazanımın API tasarımından kaynaklandığını göstermektedir. Tüm veri kümeleri, betikler ve ham sonuçlar tek komutla yeniden üretilebilecek biçimde yayımlanmıştır.
...
tutulan ara temsile bağlıdır. Somut sözdizimi ağacı (İng. Concrete Syntax Tree, CST) kullanan araçlar,
...
yardımcıları da uzun sözdizimli (eşleme) girdileri hata vermeden işler.
...
GoogleTest ile yazılmış ve CTest'e bağlanmıştır; paket şu anda 55 test
içermektedir.

Kütüphane kodu toplam 2.471 satırdır (1.832 satır \texttt{.cpp} kaynak
kodu ve 639 satır başlık). Değerlendirme altyapısı ayrıca beş adet C++ ölçüm aracı
...
\texttt{ComposeFile::validate()} yaygın alanlar için iş kuralı denetimleri
yapar: servis adının biçimini ve servis düğümünün eşleme olmasını;
\texttt{extends} kullanan servisler dışında \texttt{image} ya da
\texttt{build} alanlarından en az birinin bulunmasını; \texttt{ports},
\texttt{volumes}, \texttt{networks} ve \texttt{environment} gibi temel
alanların YAML türlerini; port ve volume girdilerinin biçimini ve
yinelenmesini; liste biçimli \texttt{environment} girdilerindeki ve
\texttt{networks} başvurularındaki tekrarları ve üst seviyede
\texttt{networks} bölümü bulunan dosyalarda tanımsız ağlara yapılan
başvuruları denetler. İnterpolasyon içeren değerler (örneğin
\texttt{"\$\{PORT\}:80"}) çalışma zamanında çözüldüğünden denetim dışında
bırakılır. Asıl doğrulama ise dış araçla yapılır: üretilen her
...
Karşılaştırma sürümünde geçersizleşen 552 çıktının 548'i, tırnaklı bir skalerin
...
bunların hiçbiri tip değiştirmemiştir. Karşılaştırma sürümünde ise 452 dosyada
...
\texttt{restart} için 10; \texttt{healthcheck-retries} için 13;
\texttt{network} için 14; \texttt{deploy-cpus} için 20;
...
Deneyde başarısızlık gözlenmemiştir: on özelliğin tamamında 1.467
düzenlemenin tümü tamamlanmıştır.
...
\midrule
Küçük ($<$100 satır) & 0,15 & 0,003 & 0,15 & 0,31 \\
Orta (100--500) & 0,64 & 0,007 & 0,55 & 1,21 \\
Büyük (500--2000) & 2,25 & 0,019 & 1,96 & 4,26 \\
Çok büyük ($>$2000) & 8,62 & 0,076 & 6,17 & 15,23 \\
\bottomrule
\end{tabular}
\end{table}
...
oran küçük ve büyük dosyalarda 0,96, orta dosyalarda 0,95, çok büyük
dosyalarda 0,82'dir ve fark her grupta istatistiksel olarak anlamlıdır
...
değiştirme adımının medyanı 0,80~ms'dir (59 dosyada en fazla 1,07~ms).
...
\begin{itemize}
  ...
  \item \textbf{Doğrulama tam Compose şemasını kapsamaz.}
    \texttt{validate()} yaygın alanları ve iş kurallarını denetler, ancak
    Compose şemasındaki bütün alanları ve sürüme bağlı bütün kuralları
    eksiksiz doğrulamaz; tam şema doğrulaması dış araca, yani
    \texttt{docker compose config} komutuna bırakılmıştır.
...
çözülmüş olan problemin C++ tarafındaki karşılığıdır. İkincisi, mevcut
doğrulama kapsamının Compose şemasındaki daha fazla alan ve kuralı
kapsayacak biçimde genişletilmesi. Üçüncüsü, nesne modelinin Compose
